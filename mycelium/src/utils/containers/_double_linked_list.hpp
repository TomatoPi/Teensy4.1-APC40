        ...
        if (is_anchor())
            {
                ...
                DECL_ITERATION_SENTINEL;
                for (double_linked_node* ptr=_next;
                    ptr && ptr!=this && ITERATION_SENTINEL_NOT_REACHED;
                    ptr=ptr->_next, UPDATE_ITERATION_SENTINEL
                    )
                    { ptr->dump(output_fn); }
                ACCEPT_ITERATION_SENTINEL("@dlnode::dump");
...
void anchored_list<ValueT>::deep_clear()
    {
        ...
        do
        {
            UPDATE_ITERATION_SENTINEL;
            node_type* next_ptr = ptr->_next;
            ptr->_next = ptr->_prev = ptr;
            ptr = next_ptr;

        } while (ptr != &_anchor && ITERATION_SENTINEL_NOT_REACHED);
        ACCEPT_ITERATION_SENTINEL("@anchored_list::deep_clear");
